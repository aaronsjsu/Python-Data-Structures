class Node:
    """Node in a linked list.

    This class represent a node in a linked list, a node
    could be any form of data, and each node has two references.

    Attributes:
        data: Keeps track of the data in each node.
        prev: Points to the previous node in the linked list.
        next: Points to the next node in the linked list.
    """

    def __init__(self, data):
        """Inits Node with its data.

        Args:
            data: The data to be stored in the node.
        """
        self.data = data
        self.prev = None
        self.next = None

    def __str__(self):
        """Enables node to be printed as a string of its data."""
        return str(self.data)


class LinkedList:
    """Creates a linked list.

    This class creates a doubly circular linked list using the
    Node class to store nodes and uses Node attributes (data, prev,
    and next) to manipulate the linked list.

    Attributes:
        head: The head (first) node in the linked list.
        tail: The tail (last) node in the linked list.
    """

    def __init__(self, data=None):
        """Inits the linked list.

        This initializes the linked list object. Note that a
        linked list can be iniialized with an initial value in the
        head node, it can be initialized empty, or it can be
        initialized with a list, tuple, or another LinkedList object.

        Args:
            data: The data to initialize the linked list with, default
                  value is None.
        """
        if (type(data) is list or type(data) is tuple
            or type(data) is LinkedList):
            # Deals with the case that data is a list, tuple or
            # another LinkedList object.
            new_list = LinkedList()
            for i in data:
                new_list.add_to_end(i)
            self.head = new_list.head
            self.tail = new_list.tail
        elif data is None:
            # Inits an empty LinkedList.
            self.head = None
            self.tail = None
        else:
            # Else add one node to the LinkedList.
            self.head = Node(data)
            self.tail = self.head

    def __iter__(self):
        """Allows the linked list to be iterable."""
        element = self.head
        while True: # Loops through each element in linked list.
            yield element.data
            element = element.next
            if element is self.head:
                break # Stops looping after one time through the list.

    def insert_after(self, prev_node_data, new_data):
        """Adds data after a specified node.

        This method adds a new node(s) to the linked list after the
        location of the specified node. It takes some data as
        an argument (can be a list or tuple) and fills the node(s)
        with the data.

        Args:
            prev_node_data: The specified node's data to add a new node after.
            new_data: The new data to be added to the linked list.

        Raises:
            Exception: The prev_node variable is invalid.
        """
        if prev_node_data is None: # Makes sure prev_node_data is not None.
            raise Exception("Previous node is None")
            return
        prev_element = self.head # Reference for the node to insert after.
        while prev_element.data is not prev_node_data:
            # Loops until the previous element is found in the list.
            prev_element = prev_element.next
            if prev_element is self.head:
                # Deals with the case that the previous node doesn't esist.
                raise Exception("Previous node does not exist")
                return
        if (type(new_data) is list or type(new_data) is tuple):
            # Deals with the case that a list or tuple is to be added.
            # Create a new LinkedList and then insert it into existing one.
            inner_list = LinkedList(new_data)
            prev_element.next.prev = inner_list.tail
            inner_list.tail.next = prev_element.next
            inner_list.head.prev = prev_element
            prev_element.next = inner_list.head
            if prev_element is self.tail:
                # Deals with the case that data is being inserted after tail.
                self.tail = inner_list.tail
        else:
            new_node = Node(new_data)
            new_node.next = prev_element.next
            new_node.prev = prev_element
            prev_element.next = new_node
            new_node.next.prev = new_node
            if prev_element is self.tail:
                # Deals with the case that data is being inserted after tail.
                self.tail = new_node

    def add_to_beginning(self, new_data):
        """Adds new node(s) at beginning.

        This method adds a new node(s) to the beginning of the
        linked list. It takes some data as an argument (can be a
        list or tuple) and fills the node(s) with the data.

        Args:
            new_data: The new data to be added to the linked list.
        """
        if self.head is None: # Deals with the case of an empty list.
            self.add_to_end(new_data)
            # add_to_end() can deal with an empty list case.
            return
        if (type(new_data) is list or type(new_data) is tuple):
            # Deals with the case that a list or tuple is to be added.
            # Create a new LinkedList and then add it at beginning of
            # existing one.
            inner_list = LinkedList(new_data)
            last_element = inner_list.head
            while True:
                last_element = last_element.next
                if last_element.next is inner_list.head:
                    break
            inner_list.head.prev = self.tail
            last_element.next = self.head
            self.head.prev = last_element
            self.head = inner_list.head
            self.tail.next = self.head
        else: # Else not a list or tuple.
            new_node = Node(new_data)
            self.head.prev = new_node
            new_node.prev = self.tail
            new_node.next = self.head
            self.head = new_node
            self.tail.next = self.head

    def add_to_end(self, new_data):
        """Adds new node(s) at end.

        This method adds a new node(s) to the end of the linked list.
        It takes some data as an argument (can be a list or tuple)
        and fills the node(s) with the data.

        Args:
            new_data: The new data to be added to the linked list.
        """
        if self.head is None:
            # Deals with the case that the linked list is empty.
            if (type(new_data) is list or type(new_data) is tuple):
                new_list = LinkedList(new_data)
                self.head = new_list.head
                self.tail = new_list.tail
            else:
                self.head = Node(new_data)
                self.tail = self.head
                self.head.next = self.head
                self.head.prev = self.head
        else:
            if (type(new_data) is list or type(new_data) is tuple):
                new_list = LinkedList(new_data)
                self.tail.next = new_list.head
                new_list.head.prev = self.tail
                self.head.prev = new_list.tail
                new_list.tail.next = self.head
            else:
                prev_last_element = self.tail
                self.tail = Node(new_data)
                self.tail.prev = prev_last_element
                self.tail.next = self.head
                prev_last_element.next = self.tail
                self.head.prev = self.tail

    def contains(self, data):
        """Checks if item is in the linked list.

        This method searches the linked list to see if the specified
        item is in the list. It returns True if it is, else False.

        Args:
            data: The data to check if it's in the linked listself.
        """
        if self.head is None: # Checks for empty list.
            return False
        element = self.head
        while True: # Loops through each element in the list.
            if element.data is data: # True if a match is found.
                return True
            element = element.next
            if element is self.head:
                return False # No match found, return False.

    def size(self):
        """Returns the size/length of the linked list."""
        if self.head is None: # Checks if linked list is empty
            return 0
        count = 0
        element = self.head
        while True: # Loops through each element
            count += 1 # Iterate count of elements
            element = element.next
            if element is self.head:
                return count # End of list reached

    def remove(self, data_to_remove):
        """Removes the specified node.

        This method removes the specified node from the linked list.

        Args:
            data_to_remove: The data stored in a node that is to be removed.

        Raises:
            Exception: The data_to_remove variable is invalid.
        """
        if data_to_remove is None: # Makes sure data_to_remove is not None.
            raise Exception("Item to remove is None")
            return
        elif self.contains(data_to_remove) is False:
            raise Exception("Item to remove doesn't exist")
            return
        if data_to_remove is self.head.data:
            # Deals with the case that the node to remove is the head.
            if self.size() is 1:
                # Deals with the case that the node to remove is the only
                # node in the list
                self.clear_list()
                return
            self.head = self.head.next
            self.head.prev = self.tail
            self.tail.next = self.head
            return
        element = self.head
        while element.next.data is not data_to_remove:
            # Loops until the element before the element to remove is found.
            if element.next.next is self.head:
                # Deals with the case that the node to remove doesn't exist.
                raise Exception("Item to remove does not exist")
                return
            element = element.next
        # Get rid of reference to the node to remove.
        if element.next is self.tail:
            self.tail = element
        element.next = element.next.next
        element.next.prev = element

    def peek(self, index):
        """Returns the data at a specified index.

        This method returns the data stored in a node at the specified
        index in the linked list. It does this without deleting the node.

        Args:
            index: The index of the desired node.

        Raises:
            Exception: The specified index does not exist.
        """
        if (index >= self.size() or index < 0): # Check that index is valid.
            raise Exception("Index out of bounds")
            return
        counter = 0
        element = self.head
        while True: # Loops until correct element found
            if counter is index: # True when index is reached
                return element.data
            counter += 1
            element = element.next

    def peek_first(self):
        """Returns the data in the head node."""
        return self.head.data

    def peek_last(self):
        """Returns the data in the tail/last node."""
        return self.tail.data

    def poll(self, index):
        """Returns and removes the data at a specified index.

        This method returns the data stored in a node at the specified
        index in the linked list. It then removes the node from the
        linked list.

        Args:
            index: The index of the desired node.

        Raises:
            Exception: The specified index does not exist.
        """
        if (index >= self.size() or index < 0): # Check that index is valid.
            raise Exception("Index out of bounds")
            return
        counter = 0
        element = self.head
        while counter is not index: # Loops until correct element found.
            counter += 1
            element = element.next
        temp = element # Store in a temporary variable before removing.
        self.remove(element.data)
        return temp.data

    def poll_first(self):
        """Returns and removes the head/first node data in the linked list."""
        return self.poll(0)

    def poll_last(self):
        """Returns and removes the tail/last node data in the linked list."""
        return self.poll(self.size() - 1)

    def remove_first(self):
        """Removes the head/first node in the linked list."""
        self.remove(self.peek_first())

    def remove_last(self):
        """Removes the tail/last node in the linked list."""
        self.remove(self.peek_last())

    def reverse(self):
        """Reverses the order of the linked list."""
        reversed_list = LinkedList()
        element = self.head
        while (True): # Loops through each element in linked list.
            # Add each element in reversed order to reversed_list.
            reversed_list.add_to_beginning(element.data)
            element = element.next
            if (element == self.head):
                break
        self.head = reversed_list.head
        self.tail = reversed_list.tail

    def clear_list(self):
        """Clears the linked list (makes it empty)."""
        self.head = None
        self.tail = None

    def copy_list(self):
        """Returns a copy of this LinkedList object."""
        return LinkedList(self)

    def print_list(self):
        """Prints the linked list.

        This method prints the linked list elements in order.
        """
        element = self.head
        if element is None: # Deals with the case of an empty list.
            return
        while True: # Loops through entire linked list.
            if (len(str(element.data)) > 1):
                print(str(element.data))
            else: # Prints nodes on the same line if len(data) <= 1,
                  # this is for the sake of testing.
                print(str(element.data), end=" ")
            element = element.next
            if element is self.head:
                break # Terminates after one time through list.
        print() # Adds a new line at the end

    def print_reversed_list(self):
        """Prints the linked list.

        This method prints the linked list elements in reverse order.
        """
        element = self.tail
        if element is None: # Deals with the case of an empty list.
            return
        while True: # Loops through entire linked list.
            if (len(str(element.data)) > 1):
                print(str(element.data))
            else: # Prints nodes on the same line if len(data) <= 1,
                  # this is for the sake of testing.
                print(str(element.data), end=" ")
            element = element.prev
            if element is self.tail:
                break # Terminates after one time through list.
        print() # Adds a new line at the end
